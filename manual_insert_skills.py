#!/usr/bin/env python3
"""
直接在 Render 环境中执行 SQL 插入 50 个技能
使用方法：在 Render Shell 中运行 python3 manual_insert_skills.py
"""
import os
...
print("=" * 60)
print("🦞 手动插入 50 个技能到数据库")
print("=" * 60)

# 创建数据库引擎
engine = create_engine(DATABASE_URL)
print(f"\n✅ 数据库连接成功：{DATABASE_URL[:50]}...")
...
with engine.connect() as conn:
    ...
    if count > 0:
        print(f"⚠️  数据库中已有 {count} 个技能")
        response = input("是否继续插入？(y/n): ")
        if response.lower() != 'y':
            print("已取消操作")
            exit(0)
...
with engine.connect() as conn:
    ...
    print("\n" + "=" * 60)
    print(f"🎉 批量上传完成！")
    print(f"✅ 本次成功：{success_count}/{len(skills_data)} 个技能")
    print(f"📊 数据库总计：{final_count} 个技能")
    print(f"🌐 访问平台：https://lobster-skill-platform-v2.onrender.com/skills")
    print("=" * 60)
