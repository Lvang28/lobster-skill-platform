...
if not DATABASE_URL:
    ...
    print("💡 此脚本必须在 Render 环境中运行")
    exit(1)

print("=" * 60)
print("🦞 龙虾 Skill 平台 - 数据库初始化")
print("=" * 60)
...
with engine.connect() as conn:
    ...
    if 'skills' not in tables:
        print("❌ skills 表不存在！请确保应用已成功部署并创建了数据库表")
        exit(1)
    ...
    if count > 0:
        print(f"⚠️  数据库中已有 {count} 个技能，跳过初始化")
        exit(0)
